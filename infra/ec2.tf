...
resource "aws_ssm_association" "install_tools" {
  ...
  parameters = {
    commands = join("\n", [
        ...
        "`$action = New-ScheduledTaskAction -Execute 'powershell.exe' -Argument '-NoProfile -ExecutionPolicy Bypass -Command \"`$env:API_KEY=''''${var.proxy_api_token}''''; `$env:EXTERNAL_DNS_NAME=''''${aws_route53_record.app.fqdn}''''; Set-Location ''''C:\\figma-mcp-proxy''''; .\\figma-proxy.exe *>> C:\\figma-mcp-proxy\\logs\\proxy.log 2>&1\"' -WorkingDirectory 'C:\\figma-mcp-proxy'",
        "`$trigger = New-ScheduledTaskTrigger -AtStartup",
        "`$settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -ExecutionTimeLimit (New-TimeSpan -Days 0)",
        "`$principal = New-ScheduledTaskPrincipal -UserId 'Administrator' -LogonType Interactive -RunLevel Highest",
        "Register-ScheduledTask -TaskName 'FigmaProxy' -Action `$action -Trigger `$trigger -Settings `$settings -Principal `$principal | Out-Null",
